...
**API를 CloudFront 뒤로 옮기기.**

지금 Lambda 함수 URL은 인터넷에 그대로 열려 있습니다. `AuthType`을 `AWS_IAM`으로
바꾸면 서명 없는 호출을 막을 수 있지만, 브라우저에는 서명할 자격 증명이 없습니다 —
내려보내면 개발자 도구로 꺼내 볼 수 있으니 막으려던 것이 그대로 공개됩니다.

서명할 주체를 브라우저가 아니라 **CloudFront**로 두면 해결됩니다. CloudFront는
Lambda 함수 URL에 대해 OAC를 지원합니다.

```
브라우저 → https://pixelzoom.app/api/*  →  CloudFront (SigV4 서명)  →  Lambda (AWS_IAM)
```

함수 URL 직접 호출이 막히고, API가 사이트와 같은 오리진이 되어 **CORS 설정 자체가
사라집니다.** 대신 Terraform이 늘고 `API_BASE_URL`이 바뀝니다. 자세한 득실은
저장소 밖의 `handoff.md` ⑤에 적어 두었습니다.

급하지는 않습니다. 지금 구조에서 위협은 '무단 접근'이 아니라 남용과 비용이고,
그것은 업로드 상한·화소 상한·계정 동시 실행 한도·예산 알림이 막고 있습니다.

그 밖에 남은 것:

- **일반 이미지 리사이징 경로** — 원본 CLI에는 있으나 MVP에서 뺐습니다 (ADR-005)
- **Vite 이전** — CRA를 유지한 이유는 ADR-006에 있습니다
- **`pixelzoom_core`를 공용 패키지로 승격** — 지금은 원본과의 대조 테스트로 묶인
  의도적 중복입니다 (ADR-004)
